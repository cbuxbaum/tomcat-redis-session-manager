...
public class RedisSessionManager extends ManagerBase implements Lifecycle, SessionListener {
	...
	public Session findSession(String id) throws IOException {
		...
		session = (RedisSession) sessions.get(id);    
		if (session == null)
		{
			...
			if (data != null) 
			{
				...
				session.setSessionSerializatonMetadata(container.metadata);
				session.setSessionIsPersisted(true);
				super.add(session);
			} 
			else 
			{
	...
	public void save(Session session, boolean forceSave) throws IOException
	{
		...
		try {
			jedis = acquireConnection();
			// look up wrapper, create if necessary
			error = saveInternal(jedis, (RedisSession)session, forceSave);
			if (sessions.get(session.getId()) == null)
			{
				((RedisSession)session).setSessionIsPersisted(false);
				super.add(session);   	  
			}
		} catch (IOException e) 
		{
	...
	protected boolean saveInternal(Jedis jedis, RedisSession session, boolean forceSave) throws IOException {
		Boolean error = true;
		forceSave=true;
		...
		try {
			...
			if (log.isTraceEnabled()) {
				log.trace("Session Contents [" + session.getId() + "]:");
				Enumeration en = session.getAttributeNames();
				while(en.hasMoreElements()) {
					log.trace("  " + en.nextElement());
				}
			...
			SessionSerializationMetadata sessionSerializationMetadata = session.getSessionSerializatonMetadata();
			byte[] originalSessionAttributesHash = sessionSerializationMetadata.getSessionAttributesHash();
			byte[] sessionAttributesHash = null;
			if (
					forceSave
					|| session.isDirty()
					|| null == (isCurrentSessionPersisted = session.getSessionIsPersisted())
					|| !isCurrentSessionPersisted
					|| !Arrays.equals(originalSessionAttributesHash, (sessionAttributesHash = serializer.attributesHashFrom(session)))
					) {
				...
				{
					sessionAttributesHash = serializer.attributesHashFrom(session);
				}
				...
				jedis.set(binaryId, serializer.serializeFrom(session, updatedSerializationMetadata));

				session.resetDirtyTracking();
				session.setSessionSerializatonMetadata(updatedSerializationMetadata);
				session.setSessionIsPersisted(true);
			}
			else
			{
			...
			log.trace("Setting expire timeout on session [" + session.getId() + "] to " + getMaxInactiveInterval());
	...
	public void removeFromRedis(Session session)
	{
		Jedis jedis = null;
		Boolean error = true;
	...
	public void afterRequest(HttpSession session) 
	{
		if (session != null) 
		{
			if (session instanceof StandardSessionFacade)
			{
				session = getSessionFromFacade((StandardSessionFacade)session);
				if (session instanceof RedisSession)
				{
					...
					try {
						...
						else
						{
							...
							remove(redisSession);
							throw new Exception("session invalidated");
						}
					} 
					catch (Exception e)
					{
	...
	private HttpSession getSessionFromFacade(HttpSession session)
	{
		try
		{
		...
		{
			log.error("Could not get session from facade");
		}
		return null;
	}
	...
	{
		if (sessionEvent.getType().equals(Session.SESSION_DESTROYED_EVENT))
		{
			log.debug("removing session from map - " + sessionEvent.getSession().getId());
			Session session = sessionEvent.getSession();
			if (session instanceof RedisSession)
			{
				removeFromRedis(session);
			}
			super.remove(sessionEvent.getSession());
		}
	}
}
